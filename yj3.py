...
lib_cap_list = []
# 读取csv文件
with open('d:\\Users\\tiger\\Documents\\Work\\YM\\project\\数字流域一期\\水位库容曲线拟合\\111025000.csv', "r", encoding='utf8',
          newline='') as f:
    reader = csv.reader(f)
    for row in reader:
        water_lvl_list.append(row[0])
        lib_cap_list.append(row[1])
...
# 拟合曲线
params = np.polynomial.Polynomial.fit(water_lvl, lib_cap, 8)
...
while i < max_water_lvl + 0.01:
    list_data.append((data_pid, round(i, 2), round(params(i), 2)))
    i = i + 0.01
...
with open('d:\\Users\\tiger\\Documents\\Work\\YM\\project\\数字流域一期\\水位库容曲线拟合\\111025000_polyfit.csv', 'w', encoding='utf-8', newline='') as file_obj:
    # 1:创建writer对象
...
# 曲线画图
y_lib_cap = params(water_lvl)  # 拟合y值
